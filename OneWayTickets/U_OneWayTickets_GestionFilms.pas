...
begin
  ...
  Begin
    ...
    // Test si le film exist déjà
    if not(valeurExists(FICHIER_FILMS, valeurs[0])) then
    Begin
      if ajoutUneLigne(valeurs, FICHIER_FILMS) then
        MessageDlg('Film ajouté avec succès !', mtInformation, [mbOk, mbCancel], 0)
      else
        MessageDlg('Une erreur est survenue lors de l''ajout !', mtError, [mbOk, mbCancel], 0);
    end
    else
    Begin
      MessageDlg('Ce film exist déjà !', mtError, [mbOk, mbCancel], 0);
    end;
...
begin
  ...
  if not(valeurExists(FICHIER_FILMS, edtNomFilm.Text)) then
  Begin
    ...
    if ecritDansFichier(valeurs, FICHIER_FILMS) then
      ...
      MessageDlg('Une erreur est survenue lors de la modification !', mtError, [mbOk, mbCancel], 0);

    chargeListeFilm(); // Charge la nouvelle liste
  end
  else
  Begin
    reponse:= MessageDlg('Ce nom de film est déjà prit !', mtError ,[mbOk, mbRetry, mbCancel], 0);
  end;
...
begin
  ...
  reponse:= MessageDlg('Voulez-vous vraiment supprimer ce film ?', mtWarning, [mbYes,mbNo], 0);
  ...
  Begin
    ...
    if ecritDansFichier(valeurs, FICHIER_FILMS) then
      MessageDlg('Le film a bien été supprimé !', mtInformation, [mbOk, mbCancel], 0)
    else
